...
type Props = {
  ...
  children: string | (((value: string) => string) => Node),
...
class Money extends Component<Props> {
  static query = gql`
    query MoneyQuery {
      shop {
        moneyFormat
  ...
  render() {
    return <Query query={Money.query}>{this.renderMoney}</Query>;
  }

  renderMoney = (query: Object) => {
